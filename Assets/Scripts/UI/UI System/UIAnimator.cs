    ...
    private void EnsureCanvasGroupsNow()
    {
        ...
        foreach (var g in _uiGroups)
        {
            ...
            foreach (var t in g.Targets)
            {
                ...
                {
                    RectTransform rt = t.CanvasGroup.transform as RectTransform;
                    if (rt != null)
                    {
                        Undo.RecordObject(this, "Assign RectTransform");
                        t.RectTransform = rt;
                        changedThis = true;
                    }
                }

                // CanvasGroup 자동 추가/연결
                if (t.RectTransform && !t.CanvasGroup)
                {
                    GameObject go = t.RectTransform.gameObject;
                    ...
                    if (t.CanvasGroup != cg)
                    {
                        Undo.RecordObject(this, "Assign CanvasGroup");
                        t.CanvasGroup = cg;
                        changedThis = true;
                    }
